...
async def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    if db_user:
        redis_client.setex(cache_key, 3600, "exists")
    ...
    redis_client.setex(f"token:{user.username}", 3600, token)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if cached_token:
        try:
            ...
            if exp > int(datetime.now(ZoneInfo("Asia/Kolkata")).timestamp()):
                return {"access_token": cached_token, "token_type": "bearer"}
        except JWTError:
            redis_client.delete(cache_key)  # Clear invalid token
